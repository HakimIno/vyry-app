      ...
      >
        {FILTER_TABS.map((tab) => {
          ...
          return (
            <Pressable
              key={tab.key}
              style={[styles.tab, { backgroundColor: isActive ? activeBgColor : inactiveBgColor }]}
              onPress={() => handleTabPress(tab.key)}
            >
              <ThemedText
                style={[styles.tabText, { color: isActive ? activeTextColor : inactiveTextColor }]}
              >
                {tab.label}
              </ThemedText>
            </Pressable>
        ...
        >
          <Ionicons name="add" size={18} color={inactiveTextColor} />
        </Pressable>
      </ScrollView>
    </View>
  );
