...
export async function apiFetch<T>(
  ...
  const doReq = async () => {
    ...
    try {
      ...
      if (__DEV__) {
        console.log('[HTTP] Response:', { url, status: response.status, statusText: response.statusText });
      }
      ...
      if (__DEV__) {
        console.error('[HTTP] Request error:', { url, error, errorName: error instanceof Error ? error.name : 'Unknown' });
      }
      
      if (error instanceof Error && error.name === 'AbortError') {
        throw new HttpError(408, 'Request timeout', { error: 'Request took too long to complete', url });
      }
      
      // Network errors
      if (error instanceof TypeError && error.message.includes('Network request failed')) {
        throw new HttpError(0, 'Network request failed', { 
          error: 'Cannot connect to server. Please check your network connection and ensure the API server is running.',
          url,
          hint: Platform.OS === 'android' ? 'For Android emulator, make sure the API server is accessible via 10.0.2.2' : undefined
        });
      }
